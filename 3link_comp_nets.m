...
figure(1)
subplot(3,1,1)
quiver(XY(:,1),XY(:,2),err_X(:),err_Y(:),0)
title("Anfis");
axis([-2 22 0 19]); 

subplot(3,1,2)
quiver(XY(:,1),XY(:,2),NN_err_X(:),NN_err_Y(:),0)
title("MLP");
axis([-2 22 0 19]); 
...
errs2 = abs(round(angle_errors2(:,1),3));
errs3 = abs(round(angle_errors3(:,1),3));
NN_errs1 = abs(round(NN_angle_errors1(:,1),3));
NN_errs2 = abs(round(NN_angle_errors2(:,1),3));
NN_errs3 = abs(round(NN_angle_errors3(:,1),3));

hist([errs1, errs2, errs3, NN_errs1, NN_errs2, NN_errs3]);
% axis([0 0.01 0 14000]);
legend('Anfis1', 'Anfis2', 'Anfis3', 'MLP1', 'MLP2', 'MLP3');
%% ANOVA test
close all;
